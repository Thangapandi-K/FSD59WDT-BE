...
// define the user controller for the user
const userController = {
    ...
    login: async (request, response) => {
        try {
            //get the user email and password from th request body
            const { email, password } = request.body;
            
            // check if the user exists in the database
            const user = await User.findOne({ email });

            //if the user does not exist, return the error response
            if (!user) {
                return response.status(404).send({ message: 'User not found' });
            }
            ...
            response.cookie('token', token, {
                ...
                secure: true,
                sameSite: 'none',
                expires: new Date(Date.now() + 24 * 3600000) //24hrs login
            });
    ...
    updateUser: async (request, response) => {
        try {
            ...
            if(!user) {
                return response.staus(404).send({ message: 'user not found'});
            }

            // update the user details if they are provided and if the user details
